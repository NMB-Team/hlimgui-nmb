...
/**
	Build macro utilities used by the Haxe bindings.
**/
@:noCompletion
class _ImGuiInternalMacro {
	#if macro
	/**
		Builds Haxe fields that provide flattened access to a native structure.
	**/
	public static function buildFlatStruct() {
		...
		for (field in fields) {
				...
				for (meta in field.meta) {
					switch (meta.name) {
						case ":flatten":
							...
							for (f in inject.fields)
								nfields.push(f);
							for (f in inject.props)
								props.push(f);
							module.push(inject.type);
							flattened = true;
						case ":flattenMap":
							if (!field.kind.match(FVar(_, _)))
								throw ":flattenMap requires flattened field to be a variable!";
							if (meta.params.length < 1)
								throw ":flattenMap needs an abstract enum to map with!";
							...
							switch (Context.getType(meta.params[0].toString())) {
								case TAbstract(_.get() => t, _) if (t.meta.has(":enum")
									&& t.type.followWithAbstracts().toString() == "Int"):
									var names = t.impl.get().statics.get();
									var subFields:Array<Field> = [];
									for (f in names) {
										if (f.name == "COUNT")
											continue; // Ignore the count enum
										var fakeField:Field = {
											name: f.name,
											pos: f.pos,
											access: [],
			...
			if (!flattened) {
				if (!field.access.contains(APublic) && !field.access.contains(APrivate) && !field.access.contains(AStatic))
			...
			case FVar(t, _):
				switch (t.toType().followWithAbstracts()) {
					case TAnonymous(_.get().fields => fields), TInst(_.get().fields.get() => fields, _):
						...
						for (f in fields) {
							if (!f.isPublic || f.meta.has(":deprecated") || !f.kind.match(FVar(VarAccess.AccNormal, VarAccess.AccNormal)))
								continue;
