...
HL_PRIM bool HL_NAME(drag_float)(vstring* label, float* v, float* v_speed, float* v_min, float* v_max, vstring* format, ImGuiSliderFlags* flags) {
	return ImGui::DragFloat(convertString(label), v, convertPtr(v_speed, 1.0f), convertPtr(v_min, 0.0f), convertPtr(v_max, 0.0f), convertString(format), convertPtr(flags, 0));
}
...
HL_PRIM bool HL_NAME(drag_int)(vstring* label, int* v, float* v_speed, int* v_min, int* v_max, vstring* format, ImGuiSliderFlags* flags) {
	return ImGui::DragInt(convertString(label), v, convertPtr(v_speed, 1.0f), convertPtr(v_min, 0), convertPtr(v_max, 0), convertString(format), convertPtr(flags, 0));
}
...
HL_PRIM bool HL_NAME(drag_int_range2)(vstring* label, int* v_current_min, int* v_current_max, float* v_speed, int* v_min, int* v_max, vstring* format, vstring* format_max, ImGuiSliderFlags* flags) {
	return ImGui::DragIntRange2(convertString(label), v_current_min, v_current_max, convertPtr(v_speed, 1.0f), convertPtr(v_min, 0), convertPtr(v_max, 0), convertString(format), convertString(format_max), convertPtr(flags, 0));
}
